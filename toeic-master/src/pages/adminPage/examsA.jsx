...
const ExamsA = () => {
  const [exams, setExams] = useState([]); // State để lưu danh sách kỳ thi
  ...
  const itemsPerPage = 4; // Số lượng kỳ thi hiển thị mỗi trang

  useEffect(() => {
    // Hàm lấy dữ liệu từ API
    const fetchExams = async () => {
      try {
        const response = await axios.get('http://localhost:3000/api/exams'); // Gọi API lấy kỳ thi
  ...
  if (loading) return <div style={styles.loading}>Đang tải kỳ thi...</div>; // Hiển thị khi dữ liệu đang được tải
  if (error) return <div style={styles.error}>Lỗi khi tải kỳ thi: {error}</div>; // Hiển thị khi có lỗi
  ...
  return (
    <div style={styles.container}>
      <h1 style={styles.title}>List of Exams</h1>
          ...
          <tbody>
            {currentItems.map((exam, index) => (
              <tr key={index}>
                ...
                <td style={styles.td}>{exam.Description}</td>
                <td style={styles.td}>{exam.TotalQuestions}</td>
                <td style={styles.td}>{exam.DurationInMinutes}</td>
              </tr>
            ))}
          </tbody>
        </table>
